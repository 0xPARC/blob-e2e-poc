...
fn write_elems<const N: usize>(bytes: &mut Vec<u8>, elems: &[F; N]) {
    for elem in elems {
        bytes
...
fn read_elems<const N: usize>(bytes: &mut impl Read) -> Result<[F; N]> {
...
pub struct PayloadInit {
    id: Hash,
    custom_predicate_ref: CustomPredicateRef,
    vds_root: Hash,
    state: RawValue,
}
...
impl PayloadInit {
    pub fn write_bytes(&self, buffer: &mut Vec<u8>) {
        ...
        write_elems(buffer, &self.custom_predicate_ref.batch.id().0);
        buffer
            .write_all(&(self.custom_predicate_ref.index as u8).to_le_bytes())
            .expect("vec write");
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let custom_pred_batch_id = Hash(read_elems(&mut bytes)?);
...
pub struct PayloadUpdate {
    id: Hash,
    shrinked_main_pod_proof: CompressedProof<F, C, D>,
    new_state: RawValue,
}
