...
async fn handle_update_rev(ctx: Arc<Context>, req_id: Uuid, id: i64, num: i64) -> Result<()> {
    ...
    let prover = Prover {};
    let rev_state_pod = task::spawn_blocking(move || builder.prove(&prover).unwrap()).await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_rev_state() -> anyhow::Result<()> {
        crate::log_init();
        common::load_dotenv()?;
        ...
        // let name = "00000001-00000001-membership_list";
        // let name = "test_state_pod-1";
        // let state_pod = { load_pod(Path::new(&cfg.pods_path), &name)? };
        let state_pod = {
            let mut builder = MainPodBuilder::new(&pod_config.params, &pod_config.vd_set);
            let mut helper = Helper::new(&mut builder, &pod_config.state_predicates);

            let state = dict!(32, {}).unwrap();
            let op = Op::Init;
            let op = Dictionary::from(op);
            let _op_raw = RawValue::from(op.commitment());
        ...
        let old_rev_state_pod: Option<MainPod> = None;
        let rev_state = dict!(pod_config.params.max_depth_mt_containers, {}).unwrap();
        ...
        let mut rev_helper = RevHelper::new(
            &mut builder,
            &pod_config.state_predicates,
            &pod_config.rev_predicates,
        );

        let (_rev_state, rev_st_update) =
            rev_helper.st_rev_sync(rev_state, op, st_update, old_st_rev_sync);
        builder.reveal(&rev_st_update);
        let prover = Prover {};
        ...
        let rev_state_pod = builder.prove(&prover)?;
        println!("# rev_state_pod\n:{}", rev_state_pod);
        rev_state_pod.pod.verify().unwrap();
